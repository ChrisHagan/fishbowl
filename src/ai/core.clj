...
(defn shortest-path
  ([net root nodedst children distance]
     ...
     (let [preds (dijkstra net root children distance)
...
(defn all-paths [net root children distance]
  (let [preds (dijkstra net root children distance)]
    (map (fn [cell-index]
           (build-path preds root cell-index))
         (keys net))))

(defn dijkstra [net root children distance]
